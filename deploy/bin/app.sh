...
PROC_TAG="PARKING-DATA_INPUT-BACKEND"
#-----------------------------------------------------------------------
...
JAVA_OPTS="$JAVA_OPTS -Xms1000m -Xmx1000m"
#PROC_OPTS="-Djmx.port=$JMX_PORT -DConfig=../conf/etl-service.xml -Dlogback.configurationFile=file:../conf/etl-service-log.xml"
#-----------------------------------------------------------------------

#-----------------------------------------------------------------------
# Process log file 
LOG_FILE="../log/error-sinopfs-backend.log"
#-----------------------------------------------------------------------
...
MAIN_CLASS="com.cmsr.hik.vision.CollectMain"
#-----------------------------------------------------------------------
...
start_proc(){
		...
		then
				...
				echo "${INFO_TAG} Starting ${PROC_TAG} ..."
				 set_classpath
				nohup ${JAVA_HOME}/bin/java -Diname=${PROC_TAG} ${JAVA_OPTS} ${PROC_OPTS} -cp ${CLASSPATH} ${MAIN_CLASS} > ${LOG_FILE} 2>&1 &
				 sleep 3
				 is_proc_run
				 if [ $? -eq 0 ] 
				 then
